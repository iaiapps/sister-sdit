...
@section('title', 'Preview Bulk Add Presensi Karyawan')

@section('content')
    <div class="card p-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="m-0">Preview Bulk Add Presensi Karyawan</h5>
            <a href="{{ route('presencekar.bulk-add') }}" class="btn btn-secondary btn-sm">kembali</a>
        </div>
        ...
        @if ($hasValid)
            <form method="POST" action="{{ route('presencekar.bulk-store') }}">
                @csrf
                <input type="hidden" name="data" value="{{ $rawData }}">
                <input type="hidden" name="date" value="{{ $date }}">
                <input type="hidden" name="confirmed" value="1">
                <div class="text-center mt-3">
                    <a href="{{ route('presencekar.bulk-add') }}" class="btn btn-secondary">Batal</a>
        ...
        @else
            <div class="text-center mt-3">
                <a href="{{ route('presencekar.bulk-add') }}" class="btn btn-warning">Kembali Perbaiki Data</a>
            </div>
        @endif
    </div>
@endsection
